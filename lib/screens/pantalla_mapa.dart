...
class _PantallaMapaState extends State<PantallaMapa> {
  ...
  bool _modoDebugActivo = kModoDebugApp;
          ...
          SnackBar(
            content: Row(
              children: [
                ...
                const Expanded(
                  child: Text(
                    'Toca un nodo para establecer el destino',
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          if (!esOrigen && !esDestino)
            FilledButton(
              onPressed: () {
                Navigator.of(context).pop(); // Cerrar el diálogo de información
                if (_origenSeleccionado == null) {
                  _establecerOrigen(nodoId);
                } else {
                  // Si está en modo selección de destino, regresar con el nodo seleccionado
                  if (widget.seleccionandoDestino) {
                    Navigator.pop(context,
                        nodoId); // Regresar al piso anterior con el destino
                  } else {
  ...
  int _extraerPisoDeNodoId(String nodoId) {
    // Los nodos tienen formato "P{piso}_{nombre}", ej: "P1_Entrada_1"
    if (nodoId.startsWith('P') && nodoId.length > 1) {
      final pisoStr = nodoId.substring(1, 2);
      return int.tryParse(pisoStr) ?? widget.numeroPiso;
    }
...
''';
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          // Botón Debug - Solo visible si kModoDebugApp está activado
          if (kModoDebugApp) ...[
            IconButton(
              icon: Icon(
  ...
  List<Widget> _buildLineasRuta() {
    ...
    for (int i = 0; i < _rutaActiva.length - 1; i++) {
      ...
      if (nodoOrigen.isEmpty || nodoDestino.isEmpty) {
        nodosNoEncontrados++;
        if (kModoDebugApp) {
          print('⚠️ Nodo no encontrado en _nodos: '
              '${nodoOrigen.isEmpty ? nodoOrigenId : ""} '
              '${nodoDestino.isEmpty ? nodoDestinoId : ""}');
        }
        continue;
      }
    ...
    if (kModoDebugApp && (_rutaActiva.isNotEmpty || nodosNoEncontrados > 0)) {
      print('🎨 _buildLineasRuta: ${lineas.length} líneas dibujadas, '
          '$nodosNoEncontrados nodos no encontrados, '
          'Total nodos en _nodos: ${_nodos.length}');
    }
