...
\vspace{4mm}
\begin{center}
  \includegraphics[width=0.45\textwidth]{ruta_calculada_a_estrella.jpeg}
\end{center}
...
\begin{figure}[H]
  ...
  \caption{Repositorio de GitHub del proyecto con la estructura de archivos y commits}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.2\textwidth]{mapa_con_controles_navegacion.jpeg}
...
\begin{figure}[H]
  ...
  \begin{minipage}[b]{0.2\textwidth}
    \centering
    \includegraphics[width=\textwidth]{zoom_out.jpeg}
    \caption*{Sin zoom}
  \end{minipage}
  \hfill
  \begin{minipage}[b]{0.2\textwidth}
    \centering
    \includegraphics[width=\textwidth]{zoom_in.jpeg}
    \caption*{Zoom al máximo}
  \end{minipage}
  \caption{Comparación de niveles de zoom: vista general del piso completo (izq.) vs. vista detallada con zoom máximo aplicado (der.)}
  \label{fig:zoom_demo}
\end{figure}
...
\begin{enumerate}
  ...
  \item \textbf{Soporte para temas claro y oscuro}: La aplicación incluye un sistema completo de temas que permite al usuario elegir entre modo claro, modo oscuro o seguir la configuración del sistema. Esta funcionalidad mejora la experiencia de usuario al permitir personalización según preferencias individuales y condiciones de iluminación.
\end{enumerate}

\paragraph{Sistema de temas y personalización}

La aplicación implementa un sistema robusto de gestión de temas utilizando \texttt{ThemeMode} de Flutter, que permite tres modos de operación:

\begin{itemize}
  \item \textbf{Modo Claro}: Interfaz con colores claros optimizada para ambientes bien iluminados
  \item \textbf{Modo Oscuro}: Interfaz con colores oscuros que reduce la fatiga visual en ambientes con poca luz y ahorra batería en pantallas OLED
  \item \textbf{Modo Automático}: Sigue la configuración del sistema operativo del dispositivo
\end{itemize}

El tema se gestiona a nivel de aplicación mediante un estado \texttt{ThemeMode} que se propaga a través de la jerarquía de widgets. La aplicación define dos temas completos basados en Material Design 3:

\begin{lstlisting}[language=Dart, caption={Configuración de temas}]
MaterialApp(
...
\textbf{Pantalla de Configuración.} Para facilitar el cambio de tema, se implementó una pantalla de configuración (\texttt{PantallaAjustes}) accesible desde el botón de configuración en la barra de aplicación de la pantalla de inicio. Esta pantalla incluye:

\begin{itemize}
  \item Selector de modo de tema con tres opciones mediante controles de radio
  \item Iconos representativos para cada modo (automático, claro, oscuro)
  \item Descripción explicativa de cada opción
  \item Aplicación inmediata del cambio de tema sin necesidad de reiniciar la aplicación
  \item Sección informativa mostrando la versión de la aplicación y datos institucionales
\end{itemize}
...
La aplicación implementa un sistema de clasificación de nodos mediante un \texttt{enum} que permite categorizar los diferentes tipos de ubicaciones en el mapa. Se definió el \texttt{enum TipoNodo} con once valores diferentes:

\begin{itemize}
  \item \textbf{entrada}: Puntos de acceso al edificio
  \item \textbf{pasillo}: Zonas de tránsito general
  \item \textbf{interseccion}: Puntos donde se cruzan múltiples pasillos
  \item \textbf{esquina}: Puntos de cambio de dirección
  \item \textbf{puerta}: Accesos a salas o espacios cerrados
  \item \textbf{escalera}: Conexiones verticales entre pisos
  \item \textbf{ascensor}: Elevadores para accesibilidad
  \item \textbf{bano}: Servicios higiénicos
  \item \textbf{laboratorio}: Espacios de prácticas y experimentación
  \item \textbf{salaClases}: Aulas y salas de docencia
  \item \textbf{puntoInteres}: Ubicaciones destacadas o de referencia
\end{itemize}

Cada tipo de nodo tiene asociado propiedades visuales específicas mediante una extensión del \texttt{enum} que proporciona:

\begin{lstlisting}[language=Dart, caption={Extensión de TipoNodo para propiedades visuales}]
extension TipoNodoExtension on TipoNodo {
  String get nombre {
    switch (this) {
      case TipoNodo.entrada: return 'Entrada';
      case TipoNodo.pasillo: return 'Pasillo';
      case TipoNodo.interseccion: return 'Interseccion';
      case TipoNodo.escalera: return 'Escalera';
      case TipoNodo.ascensor: return 'Ascensor';
      case TipoNodo.bano: return 'Bano';
      case TipoNodo.laboratorio: return 'Laboratorio';
      case TipoNodo.salaClases: return 'Sala de Clases';
      // ... otros casos
    }
  }
  ...
  IconData get icono {
    switch (this) {
      case TipoNodo.entrada: return Icons.door_front;
      case TipoNodo.escalera: return Icons.stairs;
      case TipoNodo.ascensor: return Icons.elevator;
      case TipoNodo.bano: return Icons.wc;
      case TipoNodo.laboratorio: return Icons.science;
      case TipoNodo.salaClases: return Icons.school;
      // ... otros casos
    }
  }
  ...
  Color get color {
    switch (this) {
      ...
      case TipoNodo.escalera: return Colors.orange;
      case TipoNodo.ascensor: return Colors.purple;
      case TipoNodo.bano: return Colors.blue;
      case TipoNodo.laboratorio: return Colors.teal;
      case TipoNodo.salaClases: return Colors.indigo;
      // ... otros casos
    }
  }
}
...
La aplicación puede inferir el tipo de nodo basándose en el identificador mediante análisis de subcadenas. Por ejemplo, si el ID contiene ``entrada'', se clasifica como \texttt{TipoNodo.entrada}; si contiene ``lab'', se clasifica como \texttt{TipoNodo.laboratorio}. Este sistema de inferencia automática facilita la asignación de tipos durante la creación de los datos del grafo y asegura consistencia en la visualización.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.4\textwidth]{ruta_calculada_a_estrella.jpeg}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth]{collage_codigos_qr_generados.jpg}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.45\textwidth]{pantalla_escaner_qr_activo.jpeg}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{confirmacion_qr_valido.jpeg}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.4\textwidth]{seleccion_origen_destino.jpeg}
...
\subsubsection{Pruebas de la aplicación}
...
\begin{itemize}
  \item El algoritmo A* fue probado en múltiples escenarios, confirmando rutas óptimas que tienen sentido con la topología real del edificio.
  \item Los mapas SVG fueron verificados para asegurar que los nodos y conexiones coinciden con los planos arquitectónicos.
  \item La integración del escáner QR fue testeada con todos los códigos generados, confirmando compatibilidad y correcta identificación de nodos.
  \item La interfaz de usuario fue evaluada para garantizar una experiencia intuitiva y fluida.
  \item Se realizaron pruebas de usabilidad con usuarios reales para obtener retroalimentación y mejorar la experiencia.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Agregar nuevos nodos a los mapas}: Incluir nuevos puntos de interés como puertas de emergencia, zonas de descanso y servicios adicionales.
  \item \textbf{Optimizar la interfaz de usuario}: Mejorar la experiencia visual y de interacción, incorporando animaciones suaves, transiciones y un diseño más atractivo.
  \item \textbf{Agregar otros edificios del campus}: Extender la aplicación para cubrir otros edificios de la universidad, creando una red de navegación completa.
\end{itemize}
...
\bibitem{flutter}
Google LLC, ``Flutter - Build apps for any screen,'' Flutter Documentation. [En línea]. Disponible en: \url{https://flutter.dev/}. [Accedido: 08-ene-2026].

\bibitem{dart}
Google LLC, ``Dart programming language,'' Dart Documentation. [En línea]. Disponible en: \url{https://dart.dev/}. [Accedido: 08-ene-2026].

\bibitem{librecad}
LibreCAD Community, ``LibreCAD - Open Source 2D-CAD,'' LibreCAD Official Website. [En línea]. Disponible en: \url{https://librecad.org/}. [Accedido: 08-ene-2026].

\bibitem{inkscape}
Inkscape Project, ``Inkscape - Draw Freely,'' Inkscape Official Website. [En línea]. Disponible en: \url{https://inkscape.org/}. [Accedido: 08-ene-2026].

\bibitem{vscode}
Microsoft Corporation, ``Visual Studio Code - Code Editing. Redefined,'' VS Code Documentation. [En línea]. Disponible en: \url{https://code.visualstudio.com/}. [Accedido: 08-ene-2026].

\bibitem{github}
GitHub, Inc., ``GitHub: Where the world builds software,'' GitHub Platform. [En línea]. Disponible en: \url{https://github.com/}. [Accedido: 08-ene-2026].

\bibitem{python}
Python Software Foundation, ``Python Programming Language,'' Python Official Website. [En línea]. Disponible en: \url{https://www.python.org/}. [Accedido: 08-ene-2026].

\bibitem{qrcode}
L. Vanden, ``qrcode - Python QR Code image generator,'' PyPI. [En línea]. Disponible en: \url{https://pypi.org/project/qrcode/}. [Accedido: 08-ene-2026].

\bibitem{pillow}
A. Clark et al., ``Pillow - Python Imaging Library,'' Pillow Documentation. [En línea]. Disponible en: \url{https://pillow.readthedocs.io/}. [Accedido: 08-ene-2026].

\bibitem{astar}
P. E. Hart, N. J. Nilsson, y B. Raphael, ``A Formal Basis for the Heuristic Determination of Minimum Cost Paths,'' \textit{IEEE Transactions on Systems Science and Cybernetics}, vol. 4, no. 2, pp. 100-107, 1968.

\bibitem{materialdesign}
Google LLC, ``Material Design - Design System,'' Material Design Guidelines. [En línea]. Disponible en: \url{https://m3.material.io/}. [Accedido: 08-ene-2026].

\bibitem{svg}
W3C, ``Scalable Vector Graphics (SVG) 1.1 Specification,'' World Wide Web Consortium. [En línea]. Disponible en: \url{https://www.w3.org/TR/SVG11/}. [Accedido: 08-ene-2026].
\end{thebibliography}
